...
class Main {
    companion object {
        ...
        private fun findCommand(command: String): Path? {
    ...
    private val transcoder = TranscoderSmart()

    private val versionJson: ByteArray = JsonObject().let { json ->
    ...
    private val loader: LoadingCache<DataSource, ByteArray> = Caffeine.newBuilder()
    ...
    private fun installGraphvizDotExecutable(graphvizDot: Path?) {
    ...
    private fun renderToResponse(source: String, response: Response) {
    ...
    private fun decodeSource(source: String): String {
        return if (source.startsWith("@startuml")) {
            source
        } else {
            transcoder.decode(source)
        }
    }

    private fun checkTools(graphvizDot: Path?) {
        installGraphvizDotExecutable(graphvizDot)
